...
from uir_drawer import UIRDrawer
...
class StreamLabel(QLabel):
    def __init__(self, parent, url):
        ...
        self.streamReq = QtNetwork.QNetworkRequest(QUrl(url))
    ...
    def state2ui(self):
        for msg in self.state:
            if msg["type"] == "text":
                ...
                label = StreamLabel(self, msg["url"])
                self.lay.addWidget(label)
                self.ws.append(label)
...
class ChatWidget(QWidget):
    ...
    def resQuesRetry(self, res):
        cent = {
            "is_teacher": True, "load": True, "message": [
                ...
                {"type": "btn", "title": "D", "action": ChatWidget.actionAnswerQues, "args": {"chat_w": self, "answer": 'D'}},
            ]
        }
        self.pop_item()
        self.add_message(cent)
    ...
    def init_ui(self):
        ...
        self.uic_w = UIRDrawer()
        self.uic_w.submited.connect(self.actionSubmitUICurve)
        
        self.irc_w = UIRDrawer()
        self.irc_w.submited.connect(self.actionSubmitIRCurve)
    ...
    def actionDrawCurveHand(chat_w, table):
        ...
        if table == "UI":
            chat_w.uic_w.tx_label = xl
            chat_w.uic_w.ty_label = yl
            chat_w.uic_w.txl = datas[xl].tolist()
            chat_w.uic_w.tyl = datas[yl].tolist()
            chat_w.uic_w.show()
        elif table == "IR":
            chat_w.irc_w.tx_label = xl
            chat_w.irc_w.ty_label = yl
            chat_w.irc_w.txl = datas[xl].tolist()
            chat_w.irc_w.tyl = datas[yl].tolist()
            chat_w.irc_w.show()
    
    def actionSubmitUICurve(self, isok, pix):
        cent = {
            "is_teacher": False, "load": False, "message": [
                {"type": "text", "data": "这是我绘制的电流和电压关系图像"},
                {"type": "img", "data": pix.toImage()}
            ]
        }
    ...
    def actionSubmitIRCurve(self, isok, pix):
        cent = {
            "is_teacher": False, "load": False, "message": [
                {"type": "text", "data": "这是我绘制的电流和电阻的关系图像"},
                {"type": "img", "data": pix.toImage()}
            ]
        }
